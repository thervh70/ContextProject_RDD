...
class MousePositionTracker implements EventTracker {
    ...
    public addDOMEvent() {
        ...
        MousePositionTracker.intervalTimer = setInterval(() => {
            ...
            this.db.post(EventFactory.mousePosition(Math.round(viewportX + this.x), Math.round(viewportY + this.y),
                Math.round(viewportX), Math.round(viewportY)), EMPTY_CALLBACK, EMPTY_CALLBACK);
            this.prevX = this.x;
            this.prevY = this.y;
